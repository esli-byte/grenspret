...
function ConclusieBanner({
  ...
  function bouwDeelTekst(): string {
    const regels: string[] = [
      `🎉 Ik bespaar ${euro(netto)} door over de grens te shoppen in ${land}!`,
      "",
    ];
    if (besparingTanken > 0) regels.push(`${vlag} Tanken: +${euro(besparingTanken)}`);
    if (besparingBoodschappen > 0) regels.push(`🛒 Boodschappen: +${euro(besparingBoodschappen)}`);
    if (reiskosten > 0) regels.push(`🚗 Reiskosten: −${euro(reiskosten)}`);
  ...
  function handleWhatsApp() {
    window.open(`https://wa.me/?text=${encodeURIComponent(bouwDeelTekst())}`, "_blank");
  }

  async function handleKopieer() {
    try {
      await navigator.clipboard.writeText(bouwDeelTekst());
      setDeelStatus("Gekopieerd!");
      setTimeout(() => setDeelStatus(null), 2500);
    } catch {
      setDeelStatus("Kon niet kopiëren");
      setTimeout(() => setDeelStatus(null), 2500);
    }
  }
    ...
    return (
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary via-primary-light to-accent p-6 text-white shadow-xl shadow-accent/20">
        ...
        <div className="relative">
          ...
          {/* Subtiele deelknoppen */}
          <div className="mt-4 flex items-center gap-2 border-t border-white/15 pt-3">
            <button
              onClick={handleWhatsApp}
              className="flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1.5 text-[11px] font-bold text-white/90 backdrop-blur-sm transition-all hover:bg-white/25 active:scale-95"
            >
            ...
            {deelStatus && (
              <span className="text-[11px] font-bold text-white/70 animate-fade-in">
                ✓ {deelStatus}
              </span>
            )}
          </div>
