...
function DeelKnoppen({
  ...
  const tekst =
    aantalHuishoudens > 1
      ? `Wij besparen ${bedrag} met ${aantalHuishoudens} huishoudens door samen over de grens te shoppen in ${land}!`
      : `Ik bespaar ${bedrag} door over de grens te shoppen in ${land}!`;
  const volledigeTekst = `${tekst} Check het zelf op ${url}`;
  ...
  function handleKopieer() {
    navigator.clipboard.writeText(volledigeTekst);
  }

  return (
    <div className="space-y-3">
      <h3 className="text-sm font-extrabold text-navy dark:text-white">Deel je besparing</h3>
      <div className="grid grid-cols-2 gap-3">
        {/* WhatsApp */}
        <button
          onClick={handleWhatsApp}
          className="flex items-center justify-center gap-2.5 rounded-2xl bg-[#25D366] px-4 py-3.5 text-white shadow-md transition-all hover:bg-[#20bd5a] hover:shadow-lg active:scale-[0.98]"
        >
        ...
        {/* Native Share (mobiel) of Kopieer (desktop) */}
        {typeof navigator !== "undefined" && "share" in navigator ? (
          <button
            onClick={handleNativeShare}
            className="flex items-center justify-center gap-2.5 rounded-2xl bg-accent px-4 py-3.5 text-white shadow-md transition-all hover:bg-accent/90 hover:shadow-lg active:scale-[0.98]"
          >
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 1 0 0 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186 9.566-5.314m-9.566 7.5 9.566 5.314m0 0a2.25 2.25 0 1 0 3.935 2.186 2.25 2.25 0 0 0-3.935-2.186Zm0-12.814a2.25 2.25 0 1 0 3.933-2.185 2.25 2.25 0 0 0-3.933 2.185Z" />
            </svg>
            <span className="text-sm font-bold">Meer delen</span>
          </button>
        ) : (
          <button
            onClick={handleKopieer}
            className="flex items-center justify-center gap-2.5 rounded-2xl bg-gray-100 px-4 py-3.5 text-navy shadow-md transition-all hover:bg-gray-200 hover:shadow-lg active:scale-[0.98] dark:bg-gray-800 dark:text-white"
          >
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.666 3.888A2.25 2.25 0 0 0 13.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 0 1-.75.75H9.75a.75.75 0 0 1-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 0 1-2.25 2.25H6.75A2.25 2.25 0 0 1 4.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 0 1 1.927-.184" />
            </svg>
            <span className="text-sm font-bold">Kopieer link</span>
          </button>
        )}
      </div>
    </div>
